...
import Excel.TransportistaExcel;
import bd.Parametro;
import bd.Proveedor;
 ...
 * @author Diego
 */
public class TransportistaExport extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try{
            ...
            List<Transportista> lista = new TTransportista().getListFiltro(mapFiltro);
            if (lista==null) throw new BaseException("ERROR","Ocurrió un error al recuperar los transportistas");
            Parametro parametro = new TParametro().getByCodigo(OptionsCfg.EXPORT_PATH);
            
            boolean generado = false;
            
            String fileName  = id_reporte==1?"transportistas.pdf":"X-FG-12 documentacion subcontratista de tte.xlsx";
            
            String filePath = parametro.getValor() + File.separator + fileName;
            
            if(id_reporte==1){
//                TransportistaPdf pdf = new TransportistaPdf(lista,filtros);
//                     generado = pdf.createPdf(filePath);
            }else {
                TransportistaExcel excel = new TransportistaExcel("Listado de Transportistas",proveedor,lista);
