...
public class PreticketPdf extends BasePdf{
    ...
    private HashMap<Integer, OptionsCfg.Option> mapUnidades = OptionsCfg.getMap(OptionsCfg.getUnidades());
    private HashMap<Integer, OptionsCfg.Option> mapIVA = OptionsCfg.getMap(OptionsCfg.getClasesIva());
    public PreticketPdf( Integer id_remito){
     ...
     public PreticketPdf(Preticket preticket){
         ...
         this.num_hojas = (this.detalle.size() / 18) + 1;
         this.hoja = 0;
    }
...
//        createHeadings(cb,93,start,cliente.getNombre_comercial());
    ...
    private void encabezado(PdfContentByte cb){
        ...
        createHeadings(cb,  420,start,String.format("Hoja %d / %d",this.hoja,this.num_hojas));
        start = 475;
        lineHeight = 11;
...
//           createHeadings(cb,205,start  ,remito.getNumero().toString());
     ...
     public static void main(String[] args){
        for(Preticket r : new TPreticket().getList()){;
